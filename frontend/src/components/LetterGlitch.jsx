...
const LetterGlitch = ({
  ...
  const updateLetters = () => {
    ...
    // Ещё больше уменьшаем количество обновлений для лучшей производительности
  ...
  const handleSmoothTransitions = () => {
    ...
    let updatedCount = 0;
    const maxUpdatesPerFrame = 20; // Ещё больше ограничиваем количество обновлений за кадр
    
    letters.current.forEach(letter => {
      if (updatedCount >= maxUpdatesPerFrame) return;
      
      if (letter.colorProgress < 1) {
        letter.colorProgress += 0.05; // Быстрее завершаем переход
